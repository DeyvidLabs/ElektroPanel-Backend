...
export class AuthController {
  ...
  async validateToken(@Req() request, @Res() response) {
    ...
    console.log('Validating token:', request.cookies);

    try{
      await this.authService.verifyJwtToken(accessToken);
      console.log('Token is valid try');
      ...
      try {
        ...
        console.log('Token is valid catch');
        return response.json({ success: true });
      } catch (error) {
        throw new UnauthorizedException('User is not authenticated.');
      }
  ...
  async googleAuthRedirect(@Req() req, @Res() response: Response) {
    ...
    return response.redirect(process.env.FRONTEND_URL || 'https://panel.deyvid.dev/error');
  }
}
